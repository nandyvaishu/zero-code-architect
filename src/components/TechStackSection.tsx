    ...
    <section id="tech-stack" className="py-20 relative">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-12">
          <div className="inline-block mb-2 px-3 py-1 bg-customBlue-50 text-customBlue-600 text-sm font-medium rounded-full">My Toolkit</div>
          ...
          {skillGroups.map((group, index) => (
            <Card key={index} className="border-none shadow-md hover:shadow-lg transition-all duration-300 group">
              <CardHeader className="pb-2 flex flex-row items-center gap-3">
                <div className="p-2 bg-customBlue-50 text-customBlue-500 rounded-md group-hover:bg-customBlue-100 transition-colors">
                  {group.icon}
                </div>
                <h3 className="text-lg font-semibold group-hover:text-customBlue-600 transition-colors">
                  {group.category}
                </h3>
              </CardHeader>
              <CardContent>
                <ul className="space-y-1">
                  {group.skills.map((skill, i) => (
                    <li key={i} className="flex items-center gap-2">
                      <span className="h-1.5 w-1.5 bg-customBlue-400 rounded-full"></span>
